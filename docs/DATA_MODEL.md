...
Neue Runden tragen `INITIAL_PASS`, `TECHNICAL_RETRY` oder `QUALITY_FOLLOW_UP`, die stabile Vertragsversion, Modell-/Promptversion, exakten Request und später exakte Response. `PENDING`, `COMPLETED`, `TECHNICAL_ERROR` und `INVALID_RESPONSE` unterscheiden Request, vollständig validiertes Ergebnis und die beiden Fehlerarten. Nur Runden 1 und 2 sind für neue Daten zulässig. Ein deferrable PostgreSQL-Constraint erzwingt für `COMPLETED` alle und nur nicht gelockten Bewertungen sowie eine lückenlose Rangfolge.

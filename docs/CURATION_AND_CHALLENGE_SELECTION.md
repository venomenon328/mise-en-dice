...
`CURATOR_PROMPT_V2` verbindlich. Jeder `NEW`,
`CARRY_OVER` und `LOCKED_CONTEXT` Candidate enthält stattdessen seinen unveränderlichen Restriktionssnapshot
(Rule-ID, Rule-Code, Text). Carry-over und Locked Context kopieren ihn unverändert. Andere Contracts oder Prompts

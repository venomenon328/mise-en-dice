...
- Reroll-Semantik,
- Challenge-Historie,
- dauerhafte Challenge-Ergebnisse einschließlich eigener Zusatz-Zutaten, persönlicher Konkretisierungen offener Requirement-Snapshots, optionaler Ergebnisfotos und des expliziten Abschlusses,
...
attempt-weit. Generator `1.2.0` entscheidet Restriktionsmodus und Regel in isolierten Candidate-Substreams vor jeder Requirement-Ziehung;
...
Der Adapter ist standardmäßig deaktiviert. Bei Aktivierung kapselt er JDA, Gateway-Lifecycle, Guild-Command-Registrierung, Interaktions-Acknowledgements und Renderer vollständig im `discord`-Modul. Für den Challenge-Lifecycle einschließlich Archivreads, Ergebnis- und Card-Commands verwendet er ausschließlich `challenge :: api`; die lesenden Zutatenpfade verwenden zusätzlich die schmalen `catalog :: api`-Projektionen `IngredientLookupQueries` und `ResultIngredientCatalogQueries`. Letztere stellt für persönliche Konkretisierungen ausschließlich literale Treffer aus dem direkten oder transitiven Refinement-Teilbaum der jeweiligen offenen Vorgabe bereit; die fachliche Prüfung und Result-Versionierung bleiben im Challenge-Core. Vor der sichtbaren Vote-Aktivierung meldet der Challenge-Pfad die erfolgreiche Auslieferung an den Phase-11B-Presentation-Handshake zurück. Card- und Ergebnisfoto-Uploads werden erst nach Guild-/Operatorprüfung aus Discord geladen und ausschließlich aus der persistierten Core-Projektion wieder ausgeliefert. Der Ergebnis-Kontextflow darf dafür einen hart begrenzten, owner-/guildgebundenen TTL-Zustand ausschließlich im Adapter halten; dies umfasst vor dem Speichern auch getrennte Konkretisierungswerte. Restart oder Ablauf invalidiert ihn, und nach erfolgreicher Speicherung werden Ursprungsnachricht und Attachment-Referenzen sofort freigegeben. Der Adapter persistiert weder Message-/Channel-/Jump-/CDN-Daten noch seinen Interaktionsentwurf. Discord-IDs bleiben opake externe Subjects, nicht Teil des Challenge-Modells.
...
- Vollständigkeit der Rollen- und Neuigkeitsdaten im aktiven Ziehungspool sowie die sparsche Beschaffbarkeitssemantik des festen Session-Elektorats.
...
Vor einem INITIAL-Attempt materialisiert das Challenge-Modul zuerst das mutable Standard-Elektorat als unveränderlichen Session-Snapshot. Katalog und sichtbare Historie werden anschließend je Attempt einmal in einer `REPEATABLE READ`-Lesetransaktion als unveränderlicher Context Snapshot materialisiert. Die öffentliche Catalog-API erhält dabei genau diese feste Personenmenge und liest keine aktuellen aktiven Participants selbst. Die PostgreSQL-Projektion verwendet bestätigte `challenge`-Zeilen mit ihren damaligen Requirement-Snapshots sowie die eine gespeicherte Snapshot-Exposition eines rerollten Offer Sets. Nur die zweite Quelle liefert exakte Codes für den Cooldown, ohne sich als Challenge oder Neuigkeitskadenz auszugeben; bloß generierte Kandidaten sind keine Exposition. Derselbe Snapshot dient allen internen Batches dieses Attempts sowie ihrer Persistenz; ein Satz oder eine spätere interne Neurunde mischt nicht unbemerkt zwei Katalogstände.
...
Issue #150 ergänzt die transportneutralen `ParticipantCommands` und `ParticipantQueries` für langlebige Identitäten sowie das mutable `default_electorate_member`. Ein neuer INITIAL-Start materialisiert dessen aktive Mitglieder vor Catalog Snapshot und Generator; dieser Session-Snapshot bleibt über Voting, Restart und Reroll autoritativ, auch nach Deaktivierung. Die Catalog-Projektion wertet nur vorhandene Werte dieser festen Menge aus; fehlende Werte sind neutral, ein vorhandenes `UNAVAILABLE` blockiert. `SelectionVotingCommands` und `SelectionVotingQueries` behalten geheime Votes, Mehrheit und persistierten Tie-Break, erzeugen oder lesen aber keine operative Challenge-Teilnahme mehr. `challenge_participation` bleibt Legacy. Der Core führt weder Discord-Transport noch eine parallele Challenge-/Historienpersistenz ein und maskiert keine unbekannten Datenbankfehler als Votingkonflikte.

Der ausgehende OpenAI-Adapter verwendet einen eigenen retryfreien `RestClient` über die Responses API. Er kann nur bei expliziter Aktivierung im Spring-Profil `production` entstehen; eine widersprüchliche Aktivierung außerhalb dieses Profils scheitert beim Start gemäß ADR 0008. `CURATOR_PROMPT_V2` und ein strenges, kandidatengebundenes JSON-Schema erlauben ausschließlich qualitative Bewertung, Rang, feste Reason-Codes und kompakte Diagnosen; Tools, Speicherung, Streaming, Background- und Conversation-Zustand sind deaktiviert. HTTP-/OpenAI-Objekte verlassen den Adapter nicht. Providerfehler werden technisch klassifiziert, während strukturwidriger Output explizit `INVALID_RESPONSE` bleibt; unbekannte Datenbank-, Generator- oder Laufzeitfehler werden nicht in fachliche Erschöpfung oder Konflikte übersetzt.

...
Für normale Änderungen sind die aktuellen PR-Prüfungen **Verify / verify** und **Deployment Verify / classify, deployment, legacy-preview** maßgeblich. Der stabile `verify`-Job führt die Ergebnisse der risikobasiert ausgewählten Verify-Lanes zusammen. Der Deployment-Klassifikator meldet `skip`, `smoke` oder `full`; der stabile `deployment`-Job bleibt in jedem Modus sichtbar. `legacy-preview` läuft ausschließlich für Pull Requests mit `full`.

Verify testet die versionierte Diffklassifikation sowie die Challenge-Card-Werkzeuge in einem leichten Preflight und startet danach die erforderlichen Maven-Lanes ohne serielle Abhängigkeit untereinander: `fast` über `-Pverify-fast`, `postgresql` über `-Pverify-postgresql` und `migration` über `-Pverify-migration`. JUnit-Tags auf der gemeinsamen Current-Schema-Basis und den migrationsspezifischen Klassen bilden die explizite Zuordnung; ungetaggte reguläre Tests sind nur unter dem automatisierten Lane-Zuordnungs-Guard `fast`. Die PostgreSQL-Lane verteilt zwei explizit markierte Laufzeitschwerpunkte und den vollständigen Rest als disjunkte Matrix-Shards auf drei Runner; der Guard schützt auch diese Zuordnung. Der finale Gate weist nicht ausgewählte Lanes als nicht anwendbar aus und schlägt bei Klassifikationsfehlern sowie bei jeder fehlgeschlagenen oder abgebrochenen erforderlichen Lane fehl. `./mvnw -DforkCount=2 clean verify` bleibt ungefiltert und vollständig; eine geeignete lokale Prüfung kann `./mvnw clean verify` verwenden.

Der Verify-Klassifikator verwendet den tatsächlichen PR-/`main`-Push-Diff und bildet gemischte Diffs als Lane-Vereinigung. Reine Dokumentation und Prozessdateien benötigen keine Maven-Lane; explizite Unit-/Fast-Tests und eng bekannte DB-unabhängige Java-Klassen benötigen `fast`; sonstiger produktiver Java-Code benötigt konservativ `fast + postgresql`; Persistenz-/DB-Konfiguration benötigt ebenfalls `fast + postgresql`; Changelogs, Production-Baseline und Migrationsressourcen benötigen `postgresql + migration`. Build-, allgemeine Testinfrastruktur-, Workflow-, Klassifikator- und Gruppierungsänderungen sowie unbekannte oder unlesbare Pfade fallen auf alle drei Lanes zurück. Test-only-Diffs werden aus ihrer expliziten Zuordnung klassifiziert. Die enge Challenge-Card-Asset-Ausnahme bleibt unverändert.

Deployment Verify klassifiziert den tatsächlichen PR-/`main`-Push-Diff weiterhin unabhängig davon fail-closed: reine Dokumentations-, Prozess- und eindeutig nichtproduktive Teständerungen verwenden `skip`; normale produktive Anwendung, Runtime-Ressourcen, Maven und Liquibase verwenden den providerfreien Preview-/PostgreSQL-Smoke; Deployment-, Docker-, Workflow-, Klassifikator- und unbekannte Änderungen verwenden den vollständigen bisherigen Lifecycle. Gemischte Diffs nehmen stets den höchsten Modus. Ein täglicher Zeitplan erzwingt unabhängig von einem Diff `full`. Die Workflows bleiben die konkreten Ausführungspfade: [Verify](../.github/workflows/verify.yml), [Deployment Verify](../.github/workflows/deployment-verify.yml).

[ADR 0011](adr/0011-risk-based-ci-verification.md) ist umgesetzt: #225 stellt den gemeinsamen PostgreSQL-17-Testserver pro Fork, logisch isolierte temporäre Datenbanken und die versionierte Production-Baseline bereit; #226 implementiert die risikobasierten parallelen Verify-Lanes samt stabilem Gate; #227 implementiert die Deployment-Verify-Modi. `./mvnw clean verify` bleibt unabhängig vom CI-Sharding ein vollständiger Prüfpfad.

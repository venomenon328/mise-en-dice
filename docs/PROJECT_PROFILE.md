...
[ADR 0011](adr/0011-risk-based-ci-verification.md) ist als Zielentscheidung angenommen und für Deployment Verify durch #227 umgesetzt. Die `fast`-/`postgresql`-/`migration`-Lanes und die gemeinsame PostgreSQL-Testinfrastruktur bleiben bis zur Umsetzung von #225/#226 Zielzustand; dafür gelten weiterhin die tatsächlich eingecheckten Verify-Tests. `./mvnw clean verify` bleibt auch nach dem späteren CI-Sharding ein vollständiger Prüfpfad.
...
Für **Verify** bleibt außerhalb der engen Ausnahmen bis zur Umsetzung von #225/#226 der vollständige Nicht-Asset-Pfad maßgeblich. **Deployment Verify** folgt bereits mit #227 der risikobasierten `skip`-/`smoke`-/`full`-Klassifikation. Die redaktionellen Fachtests/-verbote werden nicht umgeschrieben; einmalige Bestands-/Diff-QA ist kein dauerhaftes Content-Test-Oracle.

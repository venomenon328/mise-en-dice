...
[ADR 0011](adr/0011-risk-based-ci-verification.md) ist als Zielentscheidung angenommen. #225 ist mit gemeinsamem PostgreSQL-17-Testserver pro Fork, logisch isolierten temporären Datenbanken sowie der versionierten Production-Baseline umgesetzt. #227 hat Deployment Verify umgesetzt. Die `fast`-/`postgresql`-/`migration`-Lanes bleiben bis #226 Zielzustand. Bis dahin gelten weiterhin die tatsächlich eingecheckten Verify-Tests, und `./mvnw clean verify` bleibt auch nach dem späteren CI-Sharding ein vollständiger Prüfpfad.

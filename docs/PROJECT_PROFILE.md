...
Für normale Änderungen sind die aktuellen PR-Prüfungen **Verify / verify** und **Deployment Verify / classify, deployment, legacy-preview** maßgeblich. Der Deployment-Klassifikator meldet `skip`, `smoke` oder `full`; der stabile `deployment`-Job bleibt in jedem Modus sichtbar. `legacy-preview` läuft ausschließlich für Pull Requests mit `full`.

Verify führt Challenge-Card-Werkzeugtests und den vollständigen Maven-Lauf `./mvnw -DforkCount=2 clean verify` aus. Eine geeignete lokale Prüfung kann `./mvnw clean verify` verwenden. Deployment Verify klassifiziert den tatsächlichen PR-/`main`-Push-Diff fail-closed: reine Dokumentations-, Prozess- und eindeutig nichtproduktive Teständerungen verwenden `skip`; normale produktive Anwendung, Runtime-Ressourcen, Maven und Liquibase verwenden den providerfreien Preview-/PostgreSQL-Smoke; Deployment-, Docker-, Workflow-, Klassifikator- und unbekannte Änderungen verwenden den vollständigen bisherigen Lifecycle. Gemischte Diffs nehmen stets den höchsten Modus. Ein täglicher Zeitplan erzwingt unabhängig von einem Diff `full`. Die Workflows bleiben die konkreten Ausführungspfade: [Verify](../.github/workflows/verify.yml), [Deployment Verify](../.github/workflows/deployment-verify.yml).

[ADR 0011](adr/0011-risk-based-ci-verification.md) ist als Zielentscheidung angenommen. Der gestapelte #225-Arbeitsstand stellt den gemeinsamen PostgreSQL-17-Testserver pro Fork, logisch isolierte temporäre Datenbanken sowie die versionierte Production-Baseline bereit; seine finale Integration und Prüfung gegen `main` stehen noch aus. #227 hat Deployment Verify umgesetzt. Die `fast`-/`postgresql`-/`migration`-Lanes bleiben bis #226 Zielzustand. Bis dahin gelten weiterhin die tatsächlich eingecheckten Verify-Tests, und `./mvnw clean verify` bleibt auch nach dem späteren CI-Sharding ein vollständiger Prüfpfad.
...
Für **Verify** bleibt außerhalb der engen Ausnahmen bis zur Umsetzung von #226 der vollständige Nicht-Asset-Pfad maßgeblich. **Deployment Verify** folgt bereits mit #227 der risikobasierten `skip`-/`smoke`-/`full`-Klassifikation. Die redaktionellen Fachtests/-verbote werden nicht umgeschrieben; einmalige Bestands-/Diff-QA ist kein dauerhaftes Content-Test-Oracle.

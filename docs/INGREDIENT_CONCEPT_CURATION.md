...
- Die Oberfläche zeigt den kanonischen Anzeigenamen bereits zusammen mit der Notiz. Ihn nicht redundant als bloßes Satzsubjekt oder schematischen Einstieg wiederholen; eine erneute Nennung ist nur zulässig, wenn die konkrete Formulierung selbst einen klaren kulinarischen Informationsgewinn trägt.
- Wissenschaftliche beziehungsweise taxonomische Namen gehören nicht in die Kuratornotiz. Ist eine solche Bezeichnung für dieselbe kulinarische Identität und denselben Bedeutungsumfang als alternativer Such- oder Produktname sinnvoll, wird sie ausschließlich als Alias-Kandidat gemäß [`INGREDIENT_NAMING_AND_ALIASES.md`](INGREDIENT_NAMING_AND_ALIASES.md) geprüft; andernfalls bleibt sie Teil der Research-Evidenz. Sie wird weder automatisch zum Alias noch ersatzweise in der Kuratornotiz abgelegt.

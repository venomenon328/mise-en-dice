...
Bestätigte Challenges sind guild-weit über `/challenges aktuell`, `/challenges liste [seite]` und `/challenges anzeigen nummer:<n>` öffentlich abrufbar. Details zeigen die unveränderlichen historischen Vorgaben, die Einschränkung und bei Bedarf die gespeicherte Challenge-Card. Cards werden ausschließlich von Mitgliedern mit `challenge-operator-role-id` über `/challenges karte-setzen bild:<attachment> [nummer:<n>] [ersetzen:<bool>]` oder `/challenges karte-entfernen nummer:<n>` verwaltet. Der Bot lädt ein Upload-Attachment erst nach dieser Rollenprüfung, akzeptiert nur eine echte PNG-Card mit `1200 × 1200` Pixeln bis 5 MiB und speichert keine Discord-CDN-URL. Es sind weder neue Berechtigungen noch privilegierte Gateway-Intents erforderlich.

...
WORDMARK_BOX = (324, 30, 552, 129)
CHALLENGE_BASELINE = 203
OPEN_BADGE_INSET = (18, 18)
OPEN_BADGE_SIZE = (104, 27)
...
def slot(index: int, requirement: Requirement, rect: tuple[int, int, int, int], output: Path) -> str:
    ...
    if large:
        name_y = y + 442
        two_line_y = (y + 420, y + 458)
        name_class = "requirement requirement-large"
    else:
        name_y = y + 216
        two_line_y = (y + 198, y + 228)
        name_class = "requirement"

    badge = ""
    if requirement.open_concept:
        badge_x = x + OPEN_BADGE_INSET[0]
        badge_y = y + OPEN_BADGE_INSET[1]
        badge_w, badge_h = OPEN_BADGE_SIZE
        badge_center = badge_x + badge_w / 2
        badge = (
            f'<g id="slot-{index}-open-badge" data-anchor="top-left"><rect class="open-badge" x="{badge_x:g}" y="{badge_y:g}" width="{badge_w}" height="{badge_h}" rx="13.5"/>'
            f'{text("OFFEN", badge_center, badge_y + 20, "open-badge-text")}</g>'
        )
...
def svg(spec: CardSpec, output: Path) -> str:
    ...
    wordmark_x, wordmark_y, wordmark_w, wordmark_h = WORDMARK_BOX
...
<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="1200" viewBox="0 0 1200 1200"
  ...
  <style>
    ...
    .requirement {{ font-family: "Go Smallcaps", "DejaVu Sans", sans-serif; font-size: 34px; font-weight: 700; font-variant: small-caps; letter-spacing: .04em; fill: #2A140B; }}
    .requirement-large {{ font-size: 40px; }}
    .requirement-two-line {{ font-size: 29px; letter-spacing: .018em; }}
    .requirement-large.requirement-two-line {{ font-size: 33px; letter-spacing: .018em; }}
  ...
  <g id="header">
    <image id="wordmark" href="{esc(wordmark_href)}" data-asset="assets/brand/mise-en-dice-wordmark-master.png" x="{wordmark_x}" y="{wordmark_y}" width="{wordmark_w}" height="{wordmark_h}" preserveAspectRatio="xMidYMid meet"/>
    {text(f"Challenge #{spec.challenge_number:03d}", 600, CHALLENGE_BASELINE, "challenge")}
  </g>
  <g id="slots">{slots}</g>
  {rule_zone(spec)}
</svg>
...
def validate_svg(path: Path, spec: CardSpec) -> list[str]:
    ...
    for index, requirement in enumerate(spec.requirements, start=1):
        ...
        badge = next((element for element in root.iter() if element.attrib.get("id") == f"slot-{index}-open-badge"), None)
        if requirement.open_concept:
            if badge is None or badge.attrib.get("data-anchor") != "top-left":
                errors.append(f"{path}: open slot {index} needs a top-left anchored OFFEN badge")
        elif badge is not None:
            errors.append(f"{path}: concrete slot {index} must not render an OFFEN badge")
    ...
    expected_wordmark = tuple(str(value) for value in WORDMARK_BOX)
    ...
    elif (wordmark.attrib.get("x"), wordmark.attrib.get("y"), wordmark.attrib.get("width"), wordmark.attrib.get("height")) != expected_wordmark:
        errors.append(f"{path}: wordmark placement must be {WORDMARK_BOX[0]},{WORDMARK_BOX[1]} / {WORDMARK_BOX[2]}x{WORDMARK_BOX[3]}")
